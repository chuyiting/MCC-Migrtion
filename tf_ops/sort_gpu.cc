...
public:
    ...
    void Compute(OpKernelContext *context) override
    {
        ...
        OP_REQUIRES(context, inPointsTensor.dims() == 2, errors::InvalidArgument("SortPointsStep1Op expects points with the following dimensions (numPoints, pointComponents)"));
        OP_REQUIRES(context, inPointsTensor.shape().dim_size(1) == 3, errors::InvalidArgument("SortPointsStep1Op expects points with at least three components"));
        ...
        OP_REQUIRES(context, inBatchTensor.dims() == 2 && inBatchTensor.shape().dim_size(0) == inPointsTensor.shape().dim_size(0) && inBatchTensor.shape().dim_size(1) == 1, errors::InvalidArgument("SortPointsStep1Op expects as batch ids input the following dimensions (numPoints)"));
        ...
        OP_REQUIRES(context, inAABBMinTensor.dims() == 2 && inAABBMinTensor.shape().dim_size(0) == batchSize_ && inAABBMinTensor.shape().dim_size(1) == 3, errors::InvalidArgument("SortPointsStep1Op expects a minimum point of the bounding box with 3 components"));
        ...
        OP_REQUIRES(context, inAABBMaxTensor.dims() == 2 && inAABBMaxTensor.shape().dim_size(0) == batchSize_ && inAABBMaxTensor.shape().dim_size(1) == 3, errors::InvalidArgument("SortPointsStep1Op expects a maximum point of the bounding box with 3 components"));
...
public:
    ...
    void Compute(OpKernelContext *context) override
    {
        // Process input points.
        const Tensor &inPointsTensor = context->input(0); // Numpoints * 3
        OP_REQUIRES(context, inPointsTensor.dims() == 2, errors::InvalidArgument("SortPointsStep2Op expects points with the following dimensions (numPoints, pointComponents)"));
        OP_REQUIRES(context, inPointsTensor.shape().dim_size(1) == 3, errors::InvalidArgument("SortPointsStep2Op expects points with three components"));
        int numPoints = inPointsTensor.shape().dim_size(0);
        auto inPointsFlat = inPointsTensor.flat<float>();
        const float *inPointsPtr = &(inPointsFlat(0));
        ...
        OP_REQUIRES(context, inFeaturesTensor.dims() == 2, errors::InvalidArgument("SortPointsStep2Op expects features with the following dimensions (numPoints, numFeatures)"));
        OP_REQUIRES(context, inFeaturesTensor.shape().dim_size(1) > 0, errors::InvalidArgument("SortPointsStep2Op expects features with at least one component"));
        ...
        OP_REQUIRES(context, inKeysTensor.dims() == 1, errors::InvalidArgument("SortPointsStep2Op expects keys with the following dimensions (numPoints)"));
        OP_REQUIRES(context, inPointsTensor.shape().dim_size(0) == numPoints, errors::InvalidArgument("SortPointsStep2Op expects the same number of keys and points"));
        ...
        OP_REQUIRES(context, inNewIndexsTensor.dims() == 1, errors::InvalidArgument("SortPointsStep2Op expects indexs with the following dimensions (numPoints)"));
        OP_REQUIRES(context, inNewIndexsTensor.shape().dim_size(0) == numPoints, errors::InvalidArgument("SortPointsStep2Op expects the same number of indexs and points"));
        ...
        OP_REQUIRES(context, inAABBMinTensor.dims() == 2 && inAABBMinTensor.shape().dim_size(0) == batchSize_ && inAABBMinTensor.shape().dim_size(1) == 3, errors::InvalidArgument("SortPointsStep2Op expects a minimum point of the bounding box with 3 components"));
        auto inAABBMinFlat = inAABBMinTensor.flat<float>();
        const float *inAABBMinPtr = &(inAABBMinFlat(0));

        const Tensor &inAABBMaxTensor = context->input(6);
        OP_REQUIRES(context, inAABBMaxTensor.dims() == 2 && inAABBMaxTensor.shape().dim_size(0) == batchSize_ && inAABBMaxTensor.shape().dim_size(1) == 3, errors::InvalidArgument("SortPointsStep2Op expects a maximum point of the bounding box with 3 components"));
        auto inAABBMaxFlat = inAABBMaxTensor.flat<float>();
        const float *inAABBMaxPtr = &(inAABBMaxFlat(0));
...
public:
    ...
    void Compute(OpKernelContext *context) override
    {
        ...
        OP_REQUIRES(context, inNewIndexsTensor.dims() == 1, errors::InvalidArgument("SortPointsStep2GradOp expects indexs with the following dimensions (numPoints)"));
        ...
        OP_REQUIRES(context, inOutputGradTensor.dims() == 2, errors::InvalidArgument("SortPointsStep2Op expects gradients with the following dimensions (numPoints, pointComponents)"));
        OP_REQUIRES(context, inOutputGradTensor.shape().dim_size(1) >= 3, errors::InvalidArgument("SortPointsStep2Op expects gradients with at least three components"));
        ...
        OP_REQUIRES(context, inOutputFeatureGradTensor.dims() == 2, errors::InvalidArgument("SortPointsStep2Op expects gradients of features with the following dimensions (numPoints, numFeatures)"));
        OP_REQUIRES(context, inOutputFeatureGradTensor.shape().dim_size(1) > 0, errors::InvalidArgument("SortPointsStep2Op expects gradients of features with at least one component"));
...
public:
    ...
    void Compute(OpKernelContext *context) override
    {
        ...
        OP_REQUIRES(context, inNewIndexsTensor.dims() == 1, errors::InvalidArgument("SortFeaturesBackOp expects indexs with the following dimensions (numPoints)"));
        int numPoints = inNewIndexsTensor.shape().dim_size(0);
        auto inNewIndexsFlat = inNewIndexsTensor.flat<int>();
        const int *inNewIndexsPtr = &(inNewIndexsFlat(0));
        ...
        OP_REQUIRES(context, inFeaturesTensor.dims() == 2, errors::InvalidArgument("SortFeaturesBackOp expects features with the following dimensions (numPoints, numFeatures)"));
        OP_REQUIRES(context, inFeaturesTensor.shape().dim_size(1) > 0, errors::InvalidArgument("SortFeaturesBackOp expects features with at least one component"));
        int numFeatures = inFeaturesTensor.shape().dim_size(1);
        auto inFeaturesFlat = inFeaturesTensor.flat<float>();
        const float *inFeaturesPtr = &(inFeaturesFlat(0));
...
public:
    ...
    void Compute(OpKernelContext *context) override
    {
        // Process input new indexs.
        const Tensor &inNewIndexsTensor = context->input(0);
        OP_REQUIRES(context, inNewIndexsTensor.dims() == 1, errors::InvalidArgument("SortFeaturesBackGradOp expects indexs with the following dimensions (numPoints)"));
        int numPoints = inNewIndexsTensor.shape().dim_size(0);
        auto inNewIndexsFlat = inNewIndexsTensor.flat<int>();
        const int *inNewIndexsPtr = &(inNewIndexsFlat(0));
        ...
        OP_REQUIRES(context, inOutputFeatureGradTensor.dims() == 2, errors::InvalidArgument("SortFeaturesBackGradOp expects gradients of features with the following dimensions (numPoints, numFeatures)"));
        OP_REQUIRES(context, inOutputFeatureGradTensor.shape().dim_size(1) > 0, errors::InvalidArgument("SortFeaturesBackGradOp expects gradients of features with at least one component"));
        int numFeatures = inOutputFeatureGradTensor.shape().dim_size(1);
        auto inOutputFeatureGradFlat = inOutputFeatureGradTensor.flat<float>();
        const float *inOutputFeatureGradPtr = &(inOutputFeatureGradFlat(0));
...
public:
    ...
    void Compute(OpKernelContext *context) override
    {
        ...
        OP_REQUIRES(context, inStartIndexsTensor.dims() == 1, errors::InvalidArgument("TransformIndexsOp expects indexs with the following dimensions (numPoints)"));
        ...
        // Process input new indexs.
        const Tensor &inNewIndexsTensor = context->input(1);
        OP_REQUIRES(context, inNewIndexsTensor.dims() == 1, errors::InvalidArgument("TransformIndexsOp expects indexs with the following dimensions (numPoints)"));
        int numPoints = inNewIndexsTensor.shape().dim_size(0);
        auto inNewIndexsFlat = inNewIndexsTensor.flat<int>();
        const int *inNewIndexsPtr = &(inNewIndexsFlat(0));
