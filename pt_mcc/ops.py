...
def _sort_points_step2_backward(ctx, grad_out_points, grad_out_batch_ids, grad_out_features, grad_out_cell_indices):
    new_indices = ctx.saved_tensors[0]
    ptsGrad, inFeatureGrad = torch.ops.pt_mcc.sort_points_step2_grad(new_indices, grad_out_points, grad_out_features)
...
def _sort_features_backward(ctx, grad):
    ...
    grad_features = None
    if ctx.needs_input_grad[0]:
        grad_features = torch.ops.pt_mcc.sort_features_back(grad, indices)
...
def _spatial_conv_backward(ctx, grad):
    ...
    print(f'feature grad shape: {featureGrads.shape}')
    print(f'weights1 grad shape: {weights1Grads.shape}')
    print(f'weights2 grad shape: {weights2Grads.shape}')
    print(f'weights out grad shape: {weightsOutGrads.shape}')
    print(f'bias1 grad shape: {biases1Grads.shape}')
    print(f'bias2 grad shape: {biases2Grads.shape}')
    print(f'bias out grad shape: {biasesOutGrads.shape}')
